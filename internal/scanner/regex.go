...
func detectContext(line string) string {
	...
	if regexp.MustCompile(`(?i)(put|write|upload|store|save|create)`).MatchString(line) {
		return "write"
	}

	// Read operations
	if regexp.MustCompile(`(?i)(get|read|download|fetch|retrieve|load)`).MatchString(line) {
		return "read"
	}

	// List operations
	if regexp.MustCompile(`(?i)(list|ls|scan|iterate)`).MatchString(line) {
		return "list"
	}
