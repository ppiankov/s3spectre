...
- **`scan` skips vendor/dependency directories by default.** Directories named exactly `vendor`, `node_modules`, `target`, `dist`, `build`, `Pods`, `site-packages`, or `bower_components` (exact basename match, not substring) are never walked, since they hold third-party dependency code or build output rather than the operator's own application code. There is no flag to re-enable scanning them -- this is fixed default behavior, not configurable.
- **Regex-based code scanning.** The scanner uses pattern matching, not AST parsing. It will miss dynamically constructed bucket names and may produce false positives on commented-out code. In source code and JSON files specifically, bucket-name assignments (`bucket = "..."`, `BUCKET_NAME: "..."`) are only recognized when the value is a quoted string literal -- an unquoted code expression (a variable, an attribute access, a function call) is never mistaken for a literal bucket name. A small set of common documentation placeholders (`bucket`, `my-bucket`, `example-bucket`, and similar) are never reported as real references from an `s3://` URL match, since real S3 bucket names are globally unique and no production bucket is actually named one of these bare words -- this placeholder filtering applies consistently across every file type the scanner supports (source code, JSON, `.env`, YAML, and Terraform), since all five share the same underlying `s3://` URL pattern. The quote-requirement fix for bare bucket-name assignments (as opposed to `s3://` URLs) is scoped to source code and JSON only -- it does not extend to `.env` or YAML's own `KEY=value`/`key: value` bucket-name patterns, which legitimately use unquoted or quote-optional values as their normal convention. In Terraform files, a `bucket = "..."` attribute whose value still contains unresolved `${...}` interpolation syntax (e.g. `bucket = "${local.cluster_name}-landing"`) is silently skipped rather than reported -- the scanner has no HCL evaluator to resolve locals or variables, so treating the raw expression as a literal bucket name would always produce a phantom `MISSING_BUCKET` finding. The bare bucket-name assignment pattern also requires the keyword (`bucket`, `s3_bucket`, `s3_name`, and their `-`/no-separator variants) to start a standalone identifier component -- it will not match a keyword embedded as a bare suffix of a longer identifier with no separator (e.g. `opCreateBucket`, `ErrCodeNoSuchBucket` -- AWS SDK operation/error-code constants), though it still matches snake_case and kebab-case forms like `other_bucket` or `my-bucket-name` where an underscore or hyphen genuinely separates the keyword from a prefix.
...
- **AWS S3 only.** No GCP Cloud Storage, Azure Blob Storage, or S3-compatible alternatives (MinIO, R2, Spaces) -- there's no custom endpoint support, only the real AWS S3 API.
