...
- **`--group-by-tag` rollup is a summary view, not a filter.** It groups the same findings by a tag value for an ownership-level rollup (bucket count + summed risk score per tag value); it does not change which buckets are scanned or scored. Buckets missing the tag land under `untagged`, never dropped. If a bucket happens to carry the tag value literally set to `untagged`, it merges into the same rollup entry as buckets missing the tag entirely -- an accepted edge case, not disambiguated.
